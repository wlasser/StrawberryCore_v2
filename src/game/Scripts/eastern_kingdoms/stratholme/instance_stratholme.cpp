...
bool instance_stratholme::StartSlaugtherSquare()
{
    if (m_auiEncounter[TYPE_BARONESS] == SPECIAL && m_auiEncounter[TYPE_NERUB] == SPECIAL && m_auiEncounter[TYPE_PALLID] == SPECIAL)
    {
        ...
        debug_log("SSC: Instance Stratholme: Open slaugther square.");
        ...
        case TYPE_BARON_RUN:
            switch(uiData)
            {
                case IN_PROGRESS:
                    ...
                    debug_log("SSC: Instance Stratholme: Baron run in progress.");
                    break;
                case FAIL:
        ...
        case TYPE_RAMSTEIN:
            if (uiData == SPECIAL)
            {
                ...
                if (!uiCount)
                {
                    ...
                    if (Creature* pBaron = GetSingleCreatureFromStorage(NPC_BARON))
                    {
                        ...
                        debug_log("SSC: Instance Stratholme - Slaugther event: Ramstein spawned.");
                    }
                }
                else
                    debug_log("SSC: Instance Stratholme - Slaugther event: %u Abomnation left to kill.", uiCount);
            }
            ...
            if (uiData == DONE)
            {
                ...
                if (Creature* pBaron = GetSingleCreatureFromStorage(NPC_BARON))
                {
                    ...
                    debug_log("SSC: Instance Stratholme - Slaugther event: Summoned 5 guards.");
                }
            }
...
void instance_stratholme::Update(uint32 uiDiff)
{
    ...
    if (m_uiBaronRunTimer)
    {
        ...
        if (m_uiBaronRunTimer <= uiDiff)
        {
            ...
            m_uiBaronRunTimer = 0;
            debug_log("SSC: Instance Stratholme: Baron run event reached end. Event has state %u.", GetData(TYPE_BARON_RUN));
        }
        else
