...
{
    debug_log("SSC: Instance Zulaman: SetData received for type %u with data %u",uiType,uiData);
    ...
    switch (uiType)
    {
        ...
        default:
            error_log("SSC: Instance Zulaman: ERROR SetData = %u for type %u does not exist/not implemented.", uiType, uiData);
            return;
    }
...
void instance_zulaman::Update(uint32 uiDiff)
{
    if (m_auiEncounter[TYPE_EVENT_RUN] == IN_PROGRESS)
    {
        if (m_uiEventTimer <= uiDiff)
        {
            ...
            {
                debug_log("SSC: Instance Zulaman: event time reach end, event failed.");
            ...
            SetData(TYPE_RUN_EVENT_TIME, m_auiEncounter[TYPE_RUN_EVENT_TIME]);
            debug_log("SSC: Instance Zulaman: minute decrease to %u.", m_auiEncounter[TYPE_RUN_EVENT_TIME]);
