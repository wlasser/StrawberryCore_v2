...
struct boss_aranAI : public ScriptedAI
{
    ...
    {
        SpellEffectEntry const* spellEffect = Spell->GetSpellEffect(EFFECT_INDEX_0);
        ...
        if (!spellEffect || !spellEffect1 || spellEffect2)
            return;
        ...
        if ((spellEffect->Effect!=SPELL_EFFECT_INTERRUPT_CAST &&
            spellEffect1->Effect!=SPELL_EFFECT_INTERRUPT_CAST &&
            spellEffect2->Effect!=SPELL_EFFECT_INTERRUPT_CAST) || !m_creature->IsNonMeleeSpellCasted(false))
            return;
