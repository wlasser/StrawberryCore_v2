...
struct npc_barnesAI : public npc_escortAI
{
    ...
    {
        debug_log("SSC: Barnes Opera Event - Introduction complete - preparing encounter %d", m_uiEventId);
        ...
        switch(m_uiEventId)
        {
            ...
            case EVENT_RAJ:
                ...
                break;
            default:
                error_log("SSC: Barnes Opera Event - Wrong EventId set: %d", m_uiEventId);
                break;
        }
...
bool GossipSelect_npc_barnes(Player* pPlayer, Creature* pCreature, uint32 uiSender, uint32 uiAction)
{
    ...
    switch(uiAction)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+3:
            ...
            outstring_log("SSC: %s manually set Opera event to EVENT_OZ", pPlayer->GetGuidStr().c_str());
            break;
        case GOSSIP_ACTION_INFO_DEF+4:
            pPlayer->CLOSE_GOSSIP_MENU();
            if (pBarnesAI && pPlayer->isGameMaster())
                pBarnesAI->m_uiEventId = EVENT_HOOD;
            outstring_log("SSC: %s manually set Opera event to EVENT_HOOD", pPlayer->GetGuidStr().c_str());
            break;
        case GOSSIP_ACTION_INFO_DEF+5:
            pPlayer->CLOSE_GOSSIP_MENU();
            if (pBarnesAI && pPlayer->isGameMaster())
                pBarnesAI->m_uiEventId = EVENT_RAJ;
            outstring_log("SSC: %s manually set Opera event to EVENT_RAJ", pPlayer->GetGuidStr().c_str());
            break;
    }
