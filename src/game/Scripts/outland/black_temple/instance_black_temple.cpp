...
bool instance_black_temple::CanPreMotherDoorOpen()
{
    ...
    {
        debug_log("SSC: Black Temple: door to Mother Shahraz can open");
        return true;
    }

    debug_log("SSC: Black Temple: Door data to Mother Shahraz requested, cannot open yet (Encounter data: %u %u %u %u)",m_auiEncounter[2],m_auiEncounter[3],m_auiEncounter[4],m_auiEncounter[5]);
    return false;
}

void instance_black_temple::SetData(uint32 uiType, uint32 uiData)
{
    debug_log("SSC: Instance Black Temple: SetData received for type %u with data %u",uiType,uiData);
    ...
    switch(uiType)
    {
        ...
        default:
            error_log("SSC: Instance Black Temple: ERROR SetData = %u for type %u does not exist/not implemented.", uiType, uiData);
            return;
    }
