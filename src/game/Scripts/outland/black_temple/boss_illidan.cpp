...
struct npc_akama_illidanAI : public ScriptedAI
{
    ...
    void BeginDoorEvent(Player* pPlayer)
    {
        ...
        debug_log("SSC: Akama - Door event initiated by player %s", pPlayer->GetObjectGuid().GetString().c_str());
            ...
            if (m_uiChannelTimer < diff)
            {
                switch(m_uiChannelCount)
                {
                    ...
                    case 6:
                        ...
                        {
                            error_log("SSC: Akama has no waypoints to start with!");
                            return;
                        }
...
struct boss_illidan_stormrageAI : public ScriptedAI
{
    ...
    void SummonFlamesOfAzzinoth()
    {
        ...
        for(uint8 i = 0; i < 2; ++i)
        {
            ...
            if (Glaive)
            {
                ...
                {
                    error_log("SSC: Illidan Stormrage AI: Unable to summon Flame of Azzinoth (entry: 22997), please check your database");
                    EnterEvadeMode();
                }
            }
            else
            {
                error_log("SSC: Illidan Stormrage AI: Unable to summon Blade of Azzinoth (entry: 22996), please check your database");
            }
        }
    ...
    void SummonMaiev()
    {
        ...
        {
            EnterEvadeMode();
            debug_log("SSC: Unable to summon Maiev Shadowsong and enter Phase 4. Resetting Encounter.");
            error_log("SSC: Unable to summon Maiev Shadowsong (entry: 23197). Check your database to see if you have the proper SQL for Maiev Shadowsong (entry: 23197)");
        }
    }
            ...
            if (m_uiTalkTimer < uiDiff)
            {
                switch(m_uiTalkCount)                           // This is only for specialized cases
                {
                    ...
                    case 9:
                        ...
                        m_creature->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE);
                        debug_log("SSC: Black Temple: Illidan intro complete, players can attack Illidan.");
                        break;
                    case 11:
                        if (m_maievGuid)
                        {
...
{
    debug_log("SSC: Akama - Illidan Introduction started. Illidan event properly begun.");
    if (m_pInstance)
    {
        m_illidanGuid = m_pInstance->GetGuid(NPC_ILLIDAN_STORMRAGE);
...
bool GOUse_go_cage_trap(Player* pPlayer, GameObject* pGo)
{
    ...
    if (!pTrigger)
    {
        error_log("SSC: Cage Trap- Unable to find trigger. This Cage Trap is now useless");
        return false;
    }
