...
struct boss_shade_of_akamaAI : public ScriptedAI
{
    ...
    {
        debug_log("SSC: Increasing Death Count for Shade of Akama encounter");
        ++m_uiDeathChannelerCount;

        if (uiGuid)
        {
            if (m_lSorcerersGUIDList.empty())
                error_log("SSC: boss_shade_of_akamaAI attempt to remove guid %s from Sorcerers list but list is already empty", uiGuid.GetString().c_str());
            else
    ...
    void PrepareChannelers()
    {
        ...
        if (!lChannelerList.empty())
        {
            ...
            error_log("SSC: boss_shade_of_akamaAI unable to find any channelers.");
    }
        ...
        if (Creature* pShade = m_pInstance->GetSingleCreatureFromStorage(NPC_SHADE_OF_AKAMA))
        {
            if (pShade->isAlive())
            {
                if (boss_shade_of_akamaAI* pShadeAI = dynamic_cast<boss_shade_of_akamaAI*>(pShade->AI()))
                    ...
                    error_log("SSC: mob_ashtongue_channelerAI dead but unable to increment DeathCount for Shade of Akama.");
            }
        }
    }
        ...
        if (Creature* pShade = m_pInstance->GetSingleCreatureFromStorage(NPC_SHADE_OF_AKAMA))
        {
            if (pShade->isAlive())
            {
                if (boss_shade_of_akamaAI* pShadeAI = dynamic_cast<boss_shade_of_akamaAI*>(pShade->AI()))
                    ...
                    error_log("SSC: mob_ashtongue_sorcererAI dead but unable to increment DeathCount for Shade of Akama.");
            }
        }
    }
