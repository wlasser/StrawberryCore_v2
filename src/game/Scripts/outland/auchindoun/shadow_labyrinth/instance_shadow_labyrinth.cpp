...
void instance_shadow_labyrinth::OnCreatureCreate(Creature* pCreature)
{
    switch(pCreature->GetEntry())
    {
        ...
        case NPC_FEL_OVERSEER:
            ...
            debug_log("SSC: Shadow Labyrinth: counting %u Fel Overseers.", m_uiFelOverseerCount);
            break;
    }
}
...
void instance_shadow_labyrinth::SetData(uint32 uiType, uint32 uiData)
{
    switch(uiType)
    {
        ...
        case TYPE_OVERSEER:
            if (uiData != DONE)
            {
                error_log("SSC: Shadow Labyrinth: TYPE_OVERSEER did not expect other data than DONE");
                return;
            }
            if (m_uiFelOverseerCount)
            {
                ...
                if (m_uiFelOverseerCount)
                {
                    debug_log("SSC: Shadow Labyrinth: %u Fel Overseers left to kill.", m_uiFelOverseerCount);
                ...
                else
                {
                    ...
                    debug_log("SSC: Shadow Labyrinth: TYPE_OVERSEER == DONE");
                }
            }
            break;
