...
void instance_old_hillsbrad::HandleThrallRelocation()
{
    if (Creature* pThrall = GetSingleCreatureFromStorage(NPC_THRALL))
    {
        debug_log("SSC: Instance Old Hillsbrad: Thrall relocation");
    ...
    switch(uiType)
    {
        case TYPE_BARREL_DIVERSION:
        {
            if (uiData == IN_PROGRESS)
            {
                ...
                debug_log("SSC: Instance Old Hillsbrad: go_barrel_old_hillsbrad count %u", m_uiBarrelCount);
                ...
                if (m_uiBarrelCount == 5)
                {
                    ...
                    if (Player* pPlayer = GetPlayerInMap())
                        ...
                        debug_log("SSC: Instance Old Hillsbrad: SetData (Type: %u Data %u) cannot find any pPlayer.", uiType, uiData);
        ...
        case TYPE_THRALL_EVENT:
        {
            ...
            if (uiData == FAIL)
            {
                if (m_uiThrallEventCount <= 20)
                {
                    ...
                    debug_log("SSC: Instance Old Hillsbrad: Thrall event failed %u times.", m_uiThrallEventCount);
                ...
                else if (m_uiThrallEventCount > 20)
                {
                    ...
                    debug_log("SSC: Instance Old Hillsbrad: Thrall event failed %u times. Reset instance required.", m_uiThrallEventCount);
                }
            }
            else
                m_auiEncounter[TYPE_THRALL_EVENT] = uiData;

            debug_log("SSC: Instance Old Hillsbrad: Thrall escort event adjusted to data %u.",uiData);
            break;
        }
        case TYPE_THRALL_PART1:
            m_auiEncounter[TYPE_THRALL_PART1] = uiData;
            debug_log("SSC: Instance Old Hillsbrad: Thrall event part I adjusted to data %u.",uiData);
            break;
        case TYPE_THRALL_PART2:
            m_auiEncounter[TYPE_THRALL_PART2] = uiData;
            debug_log("SSC: Instance Old Hillsbrad: Thrall event part II adjusted to data %u.",uiData);
            break;
        case TYPE_THRALL_PART3:
            m_auiEncounter[TYPE_THRALL_PART3] = uiData;
            debug_log("SSC: Instance Old Hillsbrad: Thrall event part III adjusted to data %u.",uiData);
            break;
        case TYPE_THRALL_PART4:
            m_auiEncounter[TYPE_THRALL_PART4] = uiData;
            debug_log("SSC: Instance Old Hillsbrad: Thrall event part IV adjusted to data %u.",uiData);
            break;
    }
}
