...
struct npc_air_force_botsAI : public ScriptedAI
{
    npc_air_force_botsAI(Creature* pCreature) : ScriptedAI(pCreature)
    {
        ...
        if (!m_pSpawnAssoc)
            error_db_log("SSC: Creature template entry %u has ScriptName npc_air_force_bots, but it's not handled by that script", pCreature->GetEntry());
        else
        {
            ...
            if (!spawnedTemplate)
            {
                error_db_log("SSC: Creature template entry %u does not exist in DB, which is required by npc_air_force_bots", m_pSpawnAssoc->m_uiSpawnedCreatureEntry);
                m_pSpawnAssoc = NULL;
                return;
            }
        }
    ...
    Creature* SummonGuard()
    {
        ...
        {
            error_db_log("SSC: npc_air_force_bots: wasn't able to spawn creature %u", m_pSpawnAssoc->m_uiSpawnedCreatureEntry);
            m_pSpawnAssoc = NULL;
        }
    ...
    if (Event)
    {
        if (SummonPatient_Timer < diff)
        {
            ...
            switch(m_creature->GetEntry())
            {
                ...
                default:
                    error_log("SSC: Invalid entry for Triage doctor. Please check your database");
                    return;
            }
