enum Spells {
    SPELL_QUAKE = 81008, H_SPELL_QUAKE = 92631,
...
public:
    mob_crystalspawn_giant() :
            CreatureScript("mob_crystalspawn_giant") {
    }
    ...
    struct mob_crystalspawn_giantAI: public ScriptedAI {
        mob_crystalspawn_giantAI(Creature *c) :
                ScriptedAI(c) {
        }
...
public:
    mob_millhouse_manastorm() :
            CreatureScript("mob_millhouse_manastorm") {
    }
    ...
    struct mob_millhouse_manastormAI: public ScriptedAI {
        mob_millhouse_manastormAI(Creature *c) :
                ScriptedAI(c) {
        }
        ...
        void UpdateAI(const uint32 diff) {
            ...
            while (uint32 eventId = events.ExecuteEvent()) {
                ...
                case EVENT_FROSTBOLT_VOLLEY:
                    ...
                    events.RescheduleEvent(EVENT_FROSTBOLT_VOLLEY,
                            rand() % 15000);
                    return;
                case EVENT_IMPENDING_DOOM:
                    if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
                        DoCast(pTarget, SPELL_IMPENDING_DOOM);
                    events.RescheduleEvent(EVENT_IMPENDING_DOOM,
                            rand() % 15000);
                    return;
                case EVENT_SHADOWFURY:
                    if (Unit *pTarget = SelectUnit(SELECT_TARGET_RANDOM, 0))
                        DoCast(pTarget, SPELL_SHADOWFURY);
                    events.RescheduleEvent(SPELL_SHADOWFURY,
                            5000 + rand() % 15000);
                    return;
                }
            }
