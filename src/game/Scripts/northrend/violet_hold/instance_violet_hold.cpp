...
{
    debug_log("SSC: instance_violet_hold: SetData got type % u, data %u.", uiType, uiData);
...
void instance_violet_hold::SetPortalId()
{
    if (IsCurrentPortalForTrash())
    {
        ...
        debug_log("SSC: instance_violet_hold: SetPortalId %i, old was id %u.", iTemp, m_uiPortalId);

        m_uiPortalId = iTemp;
    }
    else if (GetCurrentPortalNumber() == 18)
    {
        debug_log("SSC: instance_violet_hold: SetPortalId %u (Cyanigosa), old was id %u.", 0, m_uiPortalId);
        m_uiPortalId = 0;
    }
    else
    {
        debug_log("SSC: instance_violet_hold: SetPortalId %u (is boss), old was id %u.", m_uiMaxCountPortalLoc, m_uiPortalId);
...
void instance_violet_hold::SetRandomBosses()
{
    ...
    for (std::vector<BossSpawn*>::const_iterator itr = m_vRandomBosses.begin(); itr != m_vRandomBosses.end(); ++itr)
        debug_log("SSC: instance_violet_hold first random boss is entry %u", (*itr)->uiEntry);
}
