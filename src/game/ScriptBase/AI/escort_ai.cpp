...
bool npc_escortAI::AssistPlayerInCombat(Unit* pWho)
{
    ...
    if (!(m_creature->GetCreatureInfo()->type_flags[0] & CREATURE_TYPEFLAGS_CAN_ASSIST))
        return false;
...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() != POINT_MOTION_TYPE)
        {
            debug_log("SSC: EscortAI has left combat and is now returning to CombatStartPosition.");
...
bool npc_escortAI::MoveToNextWaypoint()
{
    ...
    if (CurrentWP == WaypointList.end())
    {
        debug_log("SSC: EscortAI reached end of waypoints");
        ...
        if (m_bCanReturnToStart)
        {
            ...
            debug_log("SSC: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
            return true;
        }
    ...
    debug_log("SSC: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->uiId, CurrentWP->fX, CurrentWP->fY, CurrentWP->fZ);
...
void npc_escortAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_playerGuid && !m_creature->getVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer < uiDiff)
        {
            ...
            {
                debug_log("SSC: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 uiMoveType, uint32 uiPointId)
{
    ...
    {
        debug_log("SSC: EscortAI has returned to original position before combat");
    ...
    {
        debug_log("SSC: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
    ...
    else
    {
        ...
        {
            error_log("SSC: EscortAI for Npc %u reached waypoint out of order %u, expected %u.", m_creature->GetEntry(), uiPointId, CurrentWP->uiId);
            return;
        }

        debug_log("SSC: EscortAI waypoint %u reached.", CurrentWP->uiId);
...
{
    std::vector<ScriptPointMove> const &pPointsEntries = sScriptTexts.GetPointMoveList(m_creature->GetEntry());
...
void npc_escortAI::SetCurrentWaypoint(uint32 uiPointId)
{
    ...
    if (!bFoundWaypoint)
    {
        debug_log("SSC: EscortAI current waypoint tried to set to id %u, but doesn't exist in WaypointList", uiPointId);
        return;
    }

    m_uiWPWaitTimer = 1;

    debug_log("SSC: EscortAI current waypoint set to id %u", CurrentWP->uiId);
}
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("SSC: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("SSC: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
...
void npc_escortAI::Start(bool bRun, const Player* pPlayer, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    if (m_creature->getVictim())
    {
        error_log("SSC: EscortAI attempt to Start while in combat.");
        return;
    }

    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        error_log("SSC: EscortAI attempt to Start while already escorting.");
        return;
    }
    ...
    if (WaypointList.empty())
    {
        error_db_log("SSC: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint).");
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        debug_log("SSC: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (m_creature->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        m_creature->GetMotionMaster()->MovementExpired();
        m_creature->GetMotionMaster()->MoveIdle();
        debug_log("SSC: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }

    //disable npcflags
    m_creature->SetUInt32Value(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_NONE);

    debug_log("SSC: EscortAI started with " SIZEFMTD " waypoints. Run = %d, PlayerGuid = %s", WaypointList.size(), m_bIsRunning, m_playerGuid.GetString().c_str());
