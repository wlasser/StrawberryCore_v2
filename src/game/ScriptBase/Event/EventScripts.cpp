...
INSTANTIATE_SINGLETON_1(EventScripts);
...
bool EventScripts::CanSpellEffectStartDBScript(SpellEntry const* spellinfo, SpellEffectIndex effIdx)
{
...
void EventScripts::LoadScripts(ScriptMapMapName& scripts, const char* tablename)
{
...
void EventScripts::LoadGameObjectScripts()
{
...
void EventScripts::LoadGameObjectTemplateScripts()
{
...
void EventScripts::LoadQuestEndScripts()
{
...
void EventScripts::LoadQuestStartScripts()
{
...
void EventScripts::LoadSpellScripts()
{
...
void EventScripts::LoadEventScripts()
{
...
void EventScripts::LoadGossipScripts()
{
...
void EventScripts::LoadCreatureMovementScripts()
{
...
void EventScripts::LoadDbScriptStrings()
{
...
void EventScripts::CheckScriptTexts(ScriptMapMapName const& scripts, std::set<int32>& ids)
{
    for(ScriptMapMap::const_iterator itrMM = scripts.second.begin(); itrMM != scripts.second.end(); ++itrMM)
    {
        for(ScriptMap::const_iterator itrM = itrMM->second.begin(); itrM != itrMM->second.end(); ++itrM)
        {
            if (itrM->second.command == SCRIPT_COMMAND_TALK)
            {
                for(int i = 0; i < MAX_TEXT_ID; ++i)
                {
                    if (itrM->second.textId[i] && !sObjectMgr.GetStrawberryStringLocale(itrM->second.textId[i]))
...
bool ScriptAction::GetScriptProcessTargets(WorldObject* pOrigSource, WorldObject* pOrigTarget, WorldObject*& pFinalSource, WorldObject*& pFinalTarget)
{
    ...
    if (m_script->buddyEntry)
    {
        ...
        if (m_script->IsCreatureBuddy())
        {
            ...
            Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, true, false, m_script->searchRadius);
            Strawberry::CreatureLastSearcher<Strawberry::NearestCreatureEntryWithLiveStateInObjectRangeCheck> searcher(pCreatureBuddy, u_check);
        ...
        else
        {
            ...
            Strawberry::NearestGameObjectEntryInObjectRangeCheck u_check(*pSearcher, m_script->buddyEntry, m_script->searchRadius);
            Strawberry::GameObjectLastSearcher<Strawberry::NearestGameObjectEntryInObjectRangeCheck> searcher(pGOBuddy, u_check);
...
void EventScripts::LoadAreaTriggerScripts()
{
...
void EventScripts::LoadEventIdScripts()
{
...
void EventScripts::LoadScriptNames()
{
...
uint32 EventScripts::GetScriptId(const char *name) const
{
...
uint32 EventScripts::GetAreaTriggerScriptId(uint32 triggerId) const
{
...
uint32 EventScripts::GetEventIdScriptId(uint32 eventId) const
{
...
char const* EventScripts::GetScriptLibraryVersion() const
{
...
CreatureAI* EventScripts::GetCreatureAI(Creature* pCreature)
{
    if (!m_pGetCreatureAI)
        return NULL;
...
InstanceData* EventScripts::CreateInstanceData(Map* pMap)
{
...
bool EventScripts::OnGossipHello(Player* pPlayer, Creature* pCreature)
{
    return m_pOnGossipHello != NULL && m_pOnGossipHello(pPlayer, pCreature);
}

bool EventScripts::OnGossipHello(Player* pPlayer, GameObject* pGameObject)
{
    return m_pOnGOGossipHello != NULL && m_pOnGOGossipHello(pPlayer, pGameObject);
}

bool EventScripts::OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 sender, uint32 action, const char* code)
{
    if (code)
...
bool EventScripts::OnGossipSelect(Player* pPlayer, GameObject* pGameObject, uint32 sender, uint32 action, const char* code)
{
    if (code)
...
bool EventScripts::OnQuestAccept(Player* pPlayer, Creature* pCreature, Quest const* pQuest)
{
    return m_pOnQuestAccept != NULL && m_pOnQuestAccept(pPlayer, pCreature, pQuest);
}

bool EventScripts::OnQuestAccept(Player* pPlayer, GameObject* pGameObject, Quest const* pQuest)
{
    return m_pOnGOQuestAccept != NULL && m_pOnGOQuestAccept(pPlayer, pGameObject, pQuest);
}

bool EventScripts::OnQuestAccept(Player* pPlayer, Item* pItem, Quest const* pQuest)
{
    return m_pOnItemQuestAccept != NULL && m_pOnItemQuestAccept(pPlayer, pItem, pQuest);
}

bool EventScripts::OnQuestRewarded(Player* pPlayer, Creature* pCreature, Quest const* pQuest)
{
    return m_pOnQuestRewarded != NULL && m_pOnQuestRewarded(pPlayer, pCreature, pQuest);
}

bool EventScripts::OnQuestRewarded(Player* pPlayer, GameObject* pGameObject, Quest const* pQuest)
{
    return m_pOnGOQuestRewarded != NULL && m_pOnGOQuestRewarded(pPlayer, pGameObject, pQuest);
}

uint32 EventScripts::GetDialogStatus(Player* pPlayer, Creature* pCreature)
{
...
uint32 EventScripts::GetDialogStatus(Player* pPlayer, GameObject* pGameObject)
{
...
bool EventScripts::OnGameObjectUse(Player* pPlayer, GameObject* pGameObject)
{
    return m_pOnGOUse != NULL && m_pOnGOUse(pPlayer, pGameObject);
}

bool EventScripts::OnItemUse(Player* pPlayer, Item* pItem, SpellCastTargets const& targets)
{
    return m_pOnItemUse != NULL && m_pOnItemUse(pPlayer, pItem, targets);
}

bool EventScripts::OnAreaTrigger(Player* pPlayer, AreaTriggerEntry const* atEntry)
{
    return m_pOnAreaTrigger != NULL && m_pOnAreaTrigger(pPlayer, atEntry);
}

bool EventScripts::OnProcessEvent(uint32 eventId, Object* pSource, Object* pTarget, bool isStart)
{
    return m_pOnProcessEvent != NULL && m_pOnProcessEvent(eventId, pSource, pTarget, isStart);
}

bool EventScripts::OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex, Creature* pTarget)
{
    return m_pOnEffectDummyCreature != NULL && m_pOnEffectDummyCreature(pCaster, spellId, effIndex, pTarget);
}

bool EventScripts::OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex, GameObject* pTarget)
{
    return m_pOnEffectDummyGO != NULL && m_pOnEffectDummyGO(pCaster, spellId, effIndex, pTarget);
}

bool EventScripts::OnEffectDummy(Unit* pCaster, uint32 spellId, SpellEffectIndex effIndex, Item* pTarget)
{
    return m_pOnEffectDummyItem != NULL && m_pOnEffectDummyItem(pCaster, spellId, effIndex, pTarget);
}

bool EventScripts::OnAuraDummy(Aura const* pAura, bool apply)
{
    return m_pOnAuraDummy != NULL && m_pOnAuraDummy(pAura, apply);
}

ScriptLoadResult EventScripts::LoadScriptLibrary(const char* libName)
{
    UnloadScriptLibrary();
...
void EventScripts::UnloadScriptLibrary()
{
    if (!m_hScriptLib)
        return;
...
{
    return sEventScriptMgr.GetAreaTriggerScriptId(triggerId);
}

uint32 GetEventIdScriptId(uint32 eventId)
{
    return sEventScriptMgr.GetEventIdScriptId(eventId);
}

uint32 GetScriptId(const char *name)
{
    return sEventScriptMgr.GetScriptId(name);
}

char const* GetScriptName(uint32 id)
{
    return sEventScriptMgr.GetScriptName(id);
}

uint32 GetScriptIdsCount()
{
    return sEventScriptMgr.GetScriptIdsCount();
}
